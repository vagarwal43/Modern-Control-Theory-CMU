...
from util import *
from your_controller_lqr_astar import CustomController
from planning import Planning
from evaluation import evaluation

trajectory = getTrajectory('buggyTrace.csv')
#trajectory = getTrajectory('traj-astar.csv')
...
while driver.step() != -1:
    ...
    X, Y, xdot, ydot, psi, psidot, F, delta, obstacleX, obstacleY = \
    customController.update(timestep)

    # Update trajectory from A*
    distance = np.sqrt((X - obstacleX) ** 2 + (Y - obstacleY) ** 2)
    if distance < gap and flag == 1:
        Plan = Planning()
        trajectory = Plan.run('buggyTrace.csv')
        customController = CustomController(trajectory)
        customController.startSensors(timestep)
        flag = 0
