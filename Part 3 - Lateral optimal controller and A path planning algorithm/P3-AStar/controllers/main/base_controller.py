...
class BaseController():
    ...
    def getStates(self, timestep):
        ...
        # Receiver data from Emitter
        obstacleX, obstacleY = 40, -11 # Original position of obstacle
        if self.receiver.getQueueLength() > 0:
            message = self.receiver.getData()
            obstaclePos = struct.unpack('ff', message)
            obstacleX = obstaclePos[0]
            obstacleY = obstaclePos[1]

        return delT, X, Y, xdot, ydot, psi, psidot, obstacleX, obstacleY
