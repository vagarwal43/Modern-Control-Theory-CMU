...
class AStar(object):
    ...
    def heuristic(self, current, goal):
        ...
        Euclidean distance
        """
        dist = np.sqrt((goal.x - current.x)**2 + (goal.y-current.y)**2)
        return dist
    ...
    def d(self, current, successor):
        return np.sqrt((successor.x - current.x)**2 + (successor.y - current.y)**2)
    ...
    def plan(self, start_ind, goal_ind):
        ...
        while len(open_list):
            ...
            for successor in self.get_successor(current):
                ...
                successor.g_value = successor.parent.g_value + self.d(successor.parent,successor)
